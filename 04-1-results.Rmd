...
The number of visits and hospitalizations with each of the study's ICD-10 diagnoses recorded as the first diagnosis, are shown in Table \@ref(tab:table-icd10-results).
...
All physician contacts associated with the diagnostic codes listed in Table \@ref(tab:table-icd) were extracted for the given period. 
A total of 1,963,439 separate contacts were recorded between 298,307 individual patients and 1,266 different physicians.
The movst visits for a single individual was 212. 
The distribution in the number of contacts by age can be seen in Figure \@ref(fig:density-age).
...
include_graphics(path = "_figures/results/2018-11-16-vaccine-time.png")
```

Table \@ref(tab:vaccine-doses) shows the number of children in each birth-cohort who had received zero, one, two, and three doses of a pneumococcal conjugate vaccine by four years of age. 
...
Some children in vaccine non-eligible cohorts received one, two or three doses of pneumococcal conjugate vaccines before four years of age. This generally occured at an older age than children in the vaccine eligible cohorts Figure \@ref(fig:vaccine-age).

```{r vaccine-age, fig.cap = "Age at the time of administered pneumococcal vaccine dose by birth date"}
include_graphics(path = "_figures/results/2018-11-16-vaccine-age.png")
```
