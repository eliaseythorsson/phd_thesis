...
\frac{Incidence\ rate_{post-vaccine}}{Incidence\ rate_{pre-vaccine}} =
\frac{d_{post}/T_{post}}{d_{pre}/T_{pre}}
$$
where $d_{pre}$ and $d_{post}$ are the number of visits in the pre- and post-vaccine periods respectively, and $T_{pre}$ and $T_{post}$ are the number of person-years.
The Mantel-Haenszel method was used 

To calculate the 95% confidence interval of the IRR, the standard error of the natural logarithm of the IRR was first calculated and the intervals then constructed using the delta method

$$
95\%\ CI = [\frac{IRR}{\exp{(1.96*SE)}},\ IRR * \exp(1.96*SE)]
$$
where $SE = standard\ error\ of\ \ln(IRR) = \sqrt{1/d_{post} + 1/d_{pre}}$. 
The 
