...
source('_scripts/paper_6/paper_6_otitis_media/paper_6_otitis_media_synthetic_control_analysis.R', local = TRUE)
```

#`r country` Results

```{r sparse}
if (!is.null(names(sparse_groups[sparse_groups])) &&
    length(names(sparse_groups[sparse_groups])) != 0) {
    kable(data.frame(
    'Sparse Groups' = names(sparse_groups[sparse_groups]),
    check.names = FALSE
    ),
    align = 'c')
    }
...
source('_scripts/paper_6/paper_6_otitis_media/paper_6_otitis_media_synthetic_control_plot.R', local = TRUE)
```