...
iceland <- read_excel(
    path = "_data/paper_6/paper_6_uti_outpatient/synthetic-controls-uti-outpatient.xlsx", 
    sheet = 2
)
...
hg <- hg %>%
    filter(
        ...
        str_detect(string = main_diagnosis, pattern = "N39|N30|N10|J20|J21|J22") |
            str_detect(string = other_diagnosis, pattern = "N39|N30|N10|J20|J21|J22")) %>%
    transmute(
        id = id,
        ...
        age_group = case_when(
            ...
            TRUE ~ NA_character_),
        group = case_when(
            str_detect(string = main_diagnosis, pattern = "N39|N30|N10") ~ "N10_N39",
            str_detect(string = other_diagnosis, pattern = "N39|N30|N10") ~ "N10_N39",
...
iceland <-
    ...
    select(age_group, date, N10_N39, ach_noj, everything())

iceland <- data.frame(iceland)	
write_csv(path = "_data/paper_6/paper_6_uti_outpatient/input-data.csv", x = iceland)
