...
iceland <- read_excel(
    path = "_data/paper_6/paper_6_uti_inpatient/synthetic-controls-uti-inpatient.xls", 
    sheet = 2
)
...
lsh <- 
    lsh %>%
    filter(
        str_detect(string = icd10_code, pattern = "N39|N30|N10|J20|J21|J22"),
    ...
    mutate(
        ...
        age_group = case_when(
            ...
            TRUE ~ "remove"),
        group = case_when(
            str_detect(string = icd10_code, pattern = "N39|N30|N10") ~ "N10_N39",
...
iceland <-
    iceland %>%
    left_join(lsh) %>%
    select("age_group", "date", "N10_N39", "ach_noj", "A10-B99", "C00-D48", "D50-89", "E00-99", 
           "G00-G99", "H00-99", "I00-99", "J20_22", "K00-99", "L00-99", "M00-99", 
           "N00-99", "P00-99", "Q00-99", "R00-99", "S00-T99", "U00-99", 
           "V00-Y99", "Z00-99")

iceland <- data.frame(iceland)	
write_csv(path = "_data/paper_6/paper_6_uti_inpatient/input-data.csv", x = iceland)
