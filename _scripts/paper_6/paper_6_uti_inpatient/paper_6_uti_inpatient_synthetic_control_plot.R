...
for (group in groups) {
	...
	covar_plot <- ggplot(
			...
			mapping = aes_string(
				x = rep(time_points, ncol(covars.sub)),
				y = 'value',
				group = 'variable',
				alpha = alpha1
			)
	...
	#Plot predictions
	min_max <-
		c(min(c(pred_quantiles_full[, , group], outcome_plot[, group])), max(c(pred_quantiles_full[, , group], outcome_plot[, group])))
	pred_full_plot <-
		plotPred(
			pred_quantiles_full[, , group],
			time_points
			,
			post_period,
			min_max,
	...
	if (crossval) {
		...
		pred_stack_plot_agg <-
			plotPredAgg(
				ann_pred_quantiles_stack[[group]],
				time_points,
				post_period,
				min_max,
				outcome_plot[, group],
	...
	pred_sensitivity_plot <-
		plotPred(
			...
			min_max,
			outcome_plot[, group],
			sensitivity_pred_quantiles = sensitivity_pred_quantiles[[group]],
	...
	min_max <-
		c(min(rr_roll_full[, , group], rr_roll_time[, , group]),
			max(rr_roll_full[, , group], rr_roll_time[, , group]))
	rr_roll_full_plot <-
		ggplot(
			melt(as.data.frame(rr_roll_full[, , group]), id.vars = NULL),
			mapping = aes_string(
				x = rep(time_points[(length(time_points) - nrow(rr_roll_full[, , group]) + 1):length(time_points)], ncol(rr_roll_full[, , group])),
				y = 'value',
				linetype = 'variable'
			)
		) +
	...
	rr_roll_time_plot <-
		ggplot(
			melt(as.data.frame(rr_roll_time[, , group]), id.vars = NULL),
			mapping = aes_string(
				x = rep(time_points[(length(time_points) - nrow(rr_roll_time[, , group]) + 1):length(time_points)], ncol(rr_roll_time[, , group])),
				y = 'value',
				linetype = 'variable'
			)
		) +
	...
	rr_roll_pca_plot <-
		ggplot(
			melt(as.data.frame(rr_roll_pca[, , group]), id.vars = NULL),
			mapping = aes_string(
				x = rep(time_points[(length(time_points) - nrow(rr_roll_pca[, , group]) + 1):length(time_points)], ncol(rr_roll_pca[, , group])),
				y = 'value',
				linetype = 'variable'
			)
		) +
	...
	if (crossval) {
		rr_roll_stack_plot <-
			ggplot(
				melt(as.data.frame(rr_roll_stack[, , group]), id.vars = NULL),
				mapping = aes_string(
					x = rep(time_points[(length(time_points) - nrow(rr_roll_stack[, , group]) + 1):length(time_points)], ncol(rr_roll_stack[, , group])),
					y = 'value',
					linetype = 'variable'
				)
			) +
	...
	cumsum_prevented_plot <-
		ggplot(
			melt(as.data.frame(cumsum_prevented[, , group]), id.vars = NULL),
			mapping = aes_string(
				x = rep(time_points, ncol(cumsum_prevented[, , group])),
				y = 'value',
				linetype = 'variable'
			)
		) +
	...
	if (crossval) {
		cumsum_prevented_stack_plot <-
			ggplot(
				melt(as.data.frame(cumsum_prevented_stack[, , group]), id.vars = NULL),
				mapping = aes_string(
					x = rep(time_points, ncol(cumsum_prevented_stack[, , group])),
					y = 'value',
					linetype = 'variable'
				)
			) +
