...
if (run_pandoc) {
    rmarkdown::render(
        'paper_6_ipd_synthetic_control_report.Rmd',
    ...
    knitr::knit('paper_6_ipd_synthetic_control_report.Rmd', envir = environment())
    markdown::markdownToHTML(
        'paper_6_ipd_synthetic_control_report.md',
        output = paste(output_directory, 'Paper 6 IPD Synthetic Control Report.html', sep = '')
    )
    file.remove('paper_6_ipd_synthetic_control_report.md')
    unlink('figure/', recursive = TRUE)
}
source('_scripts/paper_6/paper_6_ipd/paper_6_ipd_synthetic_control_write_results.R', local = TRUE)