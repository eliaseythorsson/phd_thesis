...
iceland <- read_excel(
    "_data/paper_6/paper_6_ipd/synthetic-controls-ipd.xls", 
    sheet = "Gögnin",
...
sykl <- sykl %>%
    filter(date >= as.Date("2005-01-01"), date <= as.Date("2017-12-31")) %>%
    mutate(
...
write_csv(path = "_data/paper_6/paper_6_IPD/input-data.csv", x = iceland)
