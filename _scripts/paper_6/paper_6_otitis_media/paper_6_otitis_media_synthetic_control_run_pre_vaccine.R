...
output_directory <- '_analyses/paper_6/paper_6_otitis_media/Results_pre_vaccine'   # Directory where results will be saved.
...
end_date          <- as.Date('2010-12-01') # Indicates the date of the last data point.
pre_period        <- as.Date(c('2005-01-01', '2007-12-31')) # Range over which the data is trained for the CausalImpact model.
post_period       <- as.Date(c('2008-01-01', '2010-12-01')) # Range from the intervention date to the end date.
eval_period       <- as.Date(c('2009-01-01', '2010-12-01')) # Range over which rate ratio calculation will be performed.
...
source('_scripts/paper_6/paper_6_otitis_media/paper_6_otitis_media_synthetic_control_report.R', local = TRUE)
source('_scripts/paper_6/paper_6_otitis_media/paper_6_otitis_media_synthetic_control_write_results.R', local = TRUE) #save .csv files with output tables
