...
aom_arranged_paper6 <- ggarrange(
    plotlist = list(
        aom_number,
        ggarrange(
            plotlist =
                list(aom_standard, else_standard),
            common.legend = T,
            legend = "bottom",
            labels = c("B", "C"),
...
ggsave(
    filename = paste0("_figures/paper_6/paper_6_otitis_media/", Sys.Date(), "-aom-arranged.pdf"), 
    plot = aom_arranged_paper6,
    width = 12, height = 14, units = "cm", dpi = 600)

ggsave(
    filename = paste0("_figures/paper_6/paper_6_otitis_media/", Sys.Date(), "-aom-arranged.png"),
    plot = aom_arranged_paper6,
    width = 12, height = 14, units = "cm", dpi = 600)
...
pred_groups <- do.call(rbind.data.frame, list_groups)[-(1:7),] %>%
    ...
    left_join(prelog_data %>% select(age_group, date, H65_H73)) %>%
    ungroup() %>%
    ...
    gather(key = model, value = pred, -age_group, -date, -H65_H73, -row_number) %>%
    mutate(row_number = as.integer(row_number),
           date = as.Date(date), 
           pred = as.numeric(pred))

predict_models <- pred_groups %>%
    ggplot(aes(x = date, y = H65_H73)) +
    geom_line(aes(y = pred, color = model)) +
    geom_segment(aes(xend = date, y = pred, yend = H65_H73), size = 0.3) +
    geom_point(size = 0.3) + 
    geom_vline(aes(xintercept = as.Date("2011-01-01")), lty = 2) +
...
ggsave(
    filename = paste0("_figures/paper_6/paper_6_otitis_media/", Sys.Date(), "-predict-models.pdf"), 
    plot = predict_models,
    width = 12, height = 14, units = "cm", dpi = 600)

ggsave(
    filename = paste0("_figures/paper_6/paper_6_otitis_media/", Sys.Date(), "-predict-models.png"),
    plot = predict_models,
    width = 12, height = 14, units = "cm", dpi = 600)
...
ggsave(
    filename = paste0("_figures/paper_6/paper_6_otitis_media/", Sys.Date(), "-stacked-models.pdf"), 
    plot = stacked_models,
    width = 12, height = 14, units = "cm", dpi = 600)

ggsave(
    filename = paste0("_figures/paper_6/paper_6_otitis_media/", Sys.Date(), "-stacked-models.png"),
    plot = stacked_models,
    width = 12, height = 14, units = "cm", dpi = 600)
...
rolling_rate <- as.data.frame.table(rr_roll_stack, stringsAsFactors = F) %>%
    ...
    scale_color_brewer(palette = "Set1") +
    scale_fill_brewer(palette = "Set1") +
    coord_trans(y = "log10", limy = c(0.45, 1.2)) +
    labs(x = NULL, y = "Rolling rate ratio")+
    theme(legend.position = "None")


ggsave(
    filename = paste0("_figures/paper_6/paper_6_otitis_media/", Sys.Date(), "-rolling-rate.pdf"), 
    plot = rolling_rate,
    width = 12, height = 14, units = "cm", dpi = 600)

ggsave(
    filename = paste0("_figures/paper_6/paper_6_otitis_media/", Sys.Date(), "-rolling-rate.png"),
    plot = rolling_rate,
    width = 12, height = 14, units = "cm", dpi = 600)
...
ggsave(
    filename = paste0("_figures/paper_6/paper_6_otitis_media/", Sys.Date(), "-cum-arranged.pdf"), 
    plot = arranged_cum_prevented,
    width = 12, height = 14, units = "cm", dpi = 600)

ggsave(
    filename = paste0("_figures/paper_6/paper_6_otitis_media/", Sys.Date(), "-cum-arranged.png"),
    plot = arranged_cum_prevented,
    width = 12, height = 14, units = "cm", dpi = 600)
