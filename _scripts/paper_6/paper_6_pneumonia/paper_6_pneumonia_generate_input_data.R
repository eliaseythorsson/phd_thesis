...
iceland <- read_excel(
    "_data/paper_6/paper_6_pneumonia/synthetic-controls-pneumonia.xls", 
    sheet = "Gögnin",
...
lsh <- 
    lsh %>%
    filter(
        str_detect(string = icd10_code, pattern = "J12|J13|J14|J15|J16|J17|J18|J20|J21|J22"),
    ...
    mutate(
        ...
        age_group = case_when(
            ...
            TRUE ~ "remove"),
        group = case_when(
            str_detect(string = icd10_code, pattern = "J12|J13|J14|J15|J16|J17|J18") ~ "J12_18",
...
iceland <-
    iceland %>%
    left_join(lsh) %>%
    select("age_group", "date", "J12_18", "ach_noj", "A10-B99", "C00-D48", "D50-89", "E00-99", 
           "G00-G99", "H00-99", "I00-99", "J20_22", "K00-99", "L00-99", "M00-99", 
           "N00-99", "P00-99", "Q00-99", "R00-99", "S00-T99", "U00-99", 
           "V00-Y99", "Z00-99")

iceland <- data.frame(iceland)	
write_csv(path = "_data/paper_6/paper_6_pneumonia/input-data.csv", x = iceland)
