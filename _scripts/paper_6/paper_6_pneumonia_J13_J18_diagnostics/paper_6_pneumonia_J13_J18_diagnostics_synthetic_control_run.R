...
country       <- 'paper_6_pneumonia_J13_J18_diagnostics' # Country or region name.
...
input_directory  <- "_data/paper_6/paper_6_pneumonia_J13_J18_diagnostics/" # Directory (or URL) containing input data file.
file_name <- "input-data.csv"
output_directory <- '_analyses/paper_6/paper_6_pneumonia_J13_J18_diagnostics/Results'    # Directory where results will be saved.
...
outcome_name <- 'J13_18'    # Name of column containing outcome.
...
start_date        <- as.Date('2006-01-01') # Indicates the date of the first data point.
...
pre_period        <- as.Date(c('2006-01-01', '2010-12-31')) # Range over which the data is trained for the CausalImpact model.
...
source('_scripts/paper_6/paper_6_pneumonia/paper_6_pneumonia_synthetic_control_report.R', local = TRUE)
source('_scripts/paper_6/paper_6_pneumonia/paper_6_pneumonia_synthetic_control_write_results.R', local = TRUE) #save .csv files with output tables
