...
iceland <- read_excel(
    path = "_data/paper_6/paper_6_otitis_media_H66/synthetic-controls-otitis-media.xlsx", 
    sheet = 2
)
...
hg <- hg %>%
    filter(
        ...
        str_detect(string = main_diagnosis, pattern = "H66|J20|J21|J22") |
            str_detect(string = other_diagnosis, pattern = "H66|J20|J21|J22")) %>%
    transmute(
        id = id,
        ...
        age_group = case_when(
            ...
            TRUE ~ NA_character_),
        group = case_when(
            str_detect(string = main_diagnosis, pattern = "H66") ~ "H66",
            str_detect(string = other_diagnosis, pattern = "H66") ~ "H66",
...
iceland <-
    ...
    select(age_group, date, H66, ach_noj, everything())

iceland <- data.frame(iceland)	
write_csv(path = "_data/paper_6/paper_6_otitis_media_H66/input-data.csv", x = iceland)
